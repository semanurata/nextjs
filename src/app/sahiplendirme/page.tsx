  ...
  return (
    ...
    <div className="font-sans grid grid-rows-[20px_1fr_20px] items-center justify-items-center  p-8 pb-20 gap-16 sm:p-20">
      <Terminal>
        ...
        <AnimatedSpan>✔ Düzenli ve Sorumluluk Alabileceğiniz Bir Hayatınız,</AnimatedSpan>
      ...
      <div className="w-full max-w-xl bg-background/80 backdrop-blur-sm rounded-xl border p-6 shadow-sm mx-auto ">
        ...
        <p className="text-muted-foreground mt-1">Bilgilerini paylaş, sana en uygun pati dostunu bulalım.</p>
        ...
        {submitted ? (
          ...
          <form className="mt-6 space-y-4" onSubmit={onSubmit}>
            <div>
              ...
              <Input id="name" placeholder="Örn. Ayşe Yılmaz" value={name} onChange={(e) => setName(e.target.value)} />
            </div>
            <div>
              <label htmlFor="email" className="text-sm font-medium">E-posta</label>
              <Input id="email" type="email" placeholder="ornek@mail.com" value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div>
              <label htmlFor="city" className="text-sm font-medium">Şehir</label>
              <Input id="city" placeholder="Örn. İstanbul" value={city} onChange={(e) => setCity(e.target.value)} />
            </div>
            <div>
              <label htmlFor="note" className="text-sm font-medium">Not (opsiyonel)</label>
              <Input id="note" placeholder="Kısaca kendinden bahset" value={message} onChange={(e) => setMessage(e.target.value)} />
            </div>
            <div className="pt-2">
              <Button type="submit" className="rounded-full">Başvuruyu Gönder</Button>
            </div>
          </form>
        )}
      </div>
